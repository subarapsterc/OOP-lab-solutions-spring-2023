...
public class Ant {
    ...
    private void initImages() {
        for (int i = 0; i <= 23; i++) {
            try {
                if (i < 10) {
                    images[i] = ImageIO.read(new File("src/Lab09/images/tile00" + i + ".png")).getScaledInstance(width, height, Image.SCALE_FAST);
                } else {
                    images[i] = ImageIO.read(new File("src/Lab09/images/tile0" + i + ".png")).getScaledInstance(width, height, Image.SCALE_FAST);
                }
            } catch (IOException e) {
